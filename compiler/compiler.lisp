...
(defun cc-block-set-stack-pointer (ctx sp-var indent)
  (let ((tosvar (gen-uniq ctx "TOS_")))
    (cc-format indent "{")
    (cc-format (+ 1 indent) "ISObject ~A = is_stack_peek(1);" tosvar)
    (cc-format (+ 1 indent) "is_stack_set_pointer(~A);" sp-var)
    (cc-format (+ 1 indent) "is_stack_push(~A);" tosvar)
    (cc-format indent "}")))
      ...
      (cond ((property name 'longjmp-p)
             (let ((jmpbuf-var (gen-uniq ctx "jmpbuf_")))
               ...
               (dynamic-let ((*cc-indent-offset*
                              (+ 1 (dynamic *cc-indent-offset*))))
                 (cc-code ctx code))
               (when (property name 'block-name-used)
                 (cc-format 1 "~A:" (property name 'block-name-used)))
               (cc-format 1 "}")
               (cc-block-set-stack-pointer ctx sp-var 0)))
            ((property name 'block-name-used)
             (dynamic-let ((*cc-indent-offset*
                            (+ 1 (dynamic *cc-indent-offset*))))
               (cc-code ctx code))
             (cc-format 1 "~A:" (property name 'block-name-used))
             (cc-block-set-stack-pointer ctx sp-var 2)))))
  (cc-format 1 "}"))
